...
#   1. Clones pdfium + abseil + fast_float + dragonbox (via gclient-less
#      manual fetch)
...
DRAGONBOX_REF="${DRAGONBOX_REF:-HEAD}"

PDFIUM_SRC="${SRC_DIR}/pdfium"

# ---- Phase 1: fetch pdfium source + abseil + fast_float + dragonbox --------
...
# Header-only for what pdfium needs: core/fpdfapi/edit's float formatter calls
# jkj::dragonbox::to_decimal, which lives entirely in dragonbox.h. The compiled
# to_chars TU under source/ stays out of the build.
DRAGONBOX_SRC="${PDFIUM_SRC}/third_party/dragonbox/src"
clone_pinned "${DRAGONBOX_SRC}" \
    https://github.com/jk-jeon/dragonbox.git "${DRAGONBOX_REF}"
DRAGONBOX_COMMIT="$(git -C "${DRAGONBOX_SRC}" rev-parse HEAD)"

PDFIUM_SOURCE_FINGERPRINT="pdfium=${PDFIUM_COMMIT};fast_float=${FAST_FLOAT_COMMIT};abseil=${ABSEIL_COMMIT};dragonbox=${DRAGONBOX_COMMIT}"
...
if [ ! -f "${WASM_PREFIX}/lib/libpdfium.a" ] || \
    ...
    # the skia_*.cpp and rust_*.cpp families: pdfium's skia- or Rust-backed
    # alternatives to its libjpeg/libpng/libbmp codecs, compiled only under
    # gn's pdf_enable_rust_* flags (the rust_* ones #include a cxx-generated
    # *.rs.h header we have no toolchain for). Every include of their headers
    # is behind `#if defined(PDF_USE_SKIA)` / `PDF_ENABLE_RUST_*`, so dropping
    # the .cpp files is safe.
    #
...
cat > "${OUT}/pdfium/source.json" <<EOF
{
  ...
  "dependencies": {
    ...
    "dragonbox": {
      "repo": "https://github.com/jk-jeon/dragonbox.git",
      "ref": "${DRAGONBOX_REF}",
      "commit": "${DRAGONBOX_COMMIT}"
    }
  },
